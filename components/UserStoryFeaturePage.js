    ...
    <div className="w-full h-screen flex flex-col">
      <div className="h-1/2 flex items-center justify-center md:p-8 lg: overflow-y-auto bg-slate-300">
        <div className="max-w-2xl w-full text-center">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">User Story + Pain Points</h1>
            ...
            {features.length > 0 ? (
              features.map((feature, index) => (
                <div key={index} className="p-4 rounded-lg w-full">
                  ...
                  <p className="text-lg font-semibold text-centeㄑr">{feature.name}</p>
                </div>
              ))
            ) : (
      ...
      {imageUrl && (
        <div className="h-1/2 px-4 md:px-8 lg:px-12 overflow-hidden">
          <div className="h-full overflow-hidden shadow-lg">
            <img src={imageUrl} alt="Browser Mockup" className="w-full h-full object-cover" />
          </div>
        </div>
      )}
    </div>
